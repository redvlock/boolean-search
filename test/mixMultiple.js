...
describe('* Search with AND and OR conditions', () => {
  ...
  describe('Search without quotes and no found first single descriptor and need found boolean TO LOWERCASE FOR GET FALSE', () => {
    const search = "DOLORS OR Aliquam AND lectus".toLowerCase()

    it('Check results', () => {
      const res = bSearch(text, search)
      assert.strictEqual(res.res, false)
    })
  })

  describe('Search with quotes and no found first single descriptor and need found boolean TO LOWERCASE FOR GET FALSE', () => {
    const search = '"DOLORS" OR "Aliquam" AND "lectus"'.toLowerCase()
